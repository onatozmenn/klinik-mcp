"""Loader and query helpers for the bundled TİTCK SKRS drug snapshot.

The snapshot (``data/titck_drugs.json``) is produced by
``scripts/build_titck_snapshot.py`` from the official TİTCK SKRS E-Reçete drug
list. At runtime only the JSON is read.
"""
...
DATA_PATH = Path(__file__).parent / "data" / "titck_drugs.json"
...
def _indices() -> tuple[list[dict], dict[str, dict], dict[str, list[dict]]]:
    ...
    for drug in drugs:
        ...
        if atc:
            by_atc.setdefault(atc, []).append(drug)
    return drugs, by_barcode, by_atc
...
def find_by_atc(atc: str) -> list[dict]:
    _, _, by_atc = _indices()
    return by_atc.get(str(atc).strip(), [])


def search_by_name(query: str, limit: int = 15) -> list[dict]:
    drugs, _, _ = _indices()
